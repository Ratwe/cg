...
MAIN_TEXT_COLOR = "#ff3c38"  # "lightblue" a94dff
TEXT_COLOR = "#ff2824"
...
def parse_line(option, option_color):
    try:
        x1 = CV_WIDE / 2 + int(x1_line.get())
        y1 = CV_HEIGHT / 2 - int(y1_line.get())
        x2 = CV_WIDE / 2 + int(x2_line.get())
        y2 = CV_HEIGHT / 2 - int(y2_line.get())
    except:
        messagebox.showerror("Ошибка", "Неверно введены координаты")
        return
...
def bresenham_int(p1, p2, color, step_count=False):
    ...
    e = 2 * dy - dx  # для вещественного: e = dy/dx - 0.5
...
def steps_measure():
    ...
    if line_len <= 0:
        messagebox.showerror("Ошибка", "Длина линии должна быть больше нуля")
        return
...
if __name__ == "__main__":
    ...
    method_text = Label(win, text="Метод построения отрезка", width=43, font="-family {Consolas} -size 16",
                        bg=MAIN_TEXT_COLOR)
    ...
    method_cda = Radiobutton(text="ЦДА", font="-family {Consolas} -size 14", variable=option, value=1, bg=BOX_COLOR,
                             activebackground=BOX_COLOR, highlightbackground=BOX_COLOR)
    method_cda.place(x=CV_WIDE + 370, y=70)

    method_wu = Radiobutton(text="Ву", font="-family {Consolas} -size 14", variable=option, value=4, bg=BOX_COLOR,
                            activebackground=BOX_COLOR, highlightbackground=BOX_COLOR)
    method_wu.place(x=CV_WIDE + 370, y=105)

    method_lib = Radiobutton(text="Библиотечная", font="-family {Consolas} -size 14", variable=option, value=6,
                             bg=BOX_COLOR, activebackground=BOX_COLOR, highlightbackground=BOX_COLOR)
    method_lib.place(x=CV_WIDE + 370, y=140)

    method_bresenhem_float = Radiobutton(text="Брезенхем (float)", font="-family {Consolas} -size 14", variable=option,
                                         value=3, bg=BOX_COLOR, activebackground=BOX_COLOR,
                                         highlightbackground=BOX_COLOR)
    method_bresenhem_float.place(x=CV_WIDE + 25, y=70)

    method_bresenhem_int = Radiobutton(text="Брезенхем (int)", font="-family {Consolas} -size 14", variable=option,
    ...
    method_bresenhem_smooth = Radiobutton(text="Брезенхем (сглаживание)", font="-family {Consolas} -size 14",
                                          variable=option, value=5, bg=BOX_COLOR, activebackground=BOX_COLOR,
                                          highlightbackground=BOX_COLOR)
    method_bresenhem_smooth.place(x=CV_WIDE + 25, y=140)

    # Color

    back_box_color = Label(text="", font="-family {Consolas} -size 16", width=35, height=5, bg=BOX_COLOR)
    back_box_color.place(x=CV_WIDE + 20, y=210)

    color_text = Label(win, text="Цвет отрезка", width=35, font="-family {Consolas} -size 16", bg=MAIN_TEXT_COLOR)
    color_text.place(x=CV_WIDE + 20, y=210)

    option_color = IntVar()
    option_color.set(1)

    color_line_black = Radiobutton(text="Черный", font="-family {Consolas} -size 14", variable=option_color, value=1,
                                   bg=BOX_COLOR, activebackground=BOX_COLOR, highlightbackground=BOX_COLOR)
    color_line_black.place(x=CV_WIDE + 25, y=250)

    color_line_blue = Radiobutton(text="Синий", font="-family {Consolas} -size 14", variable=option_color, value=3,
                                  bg=BOX_COLOR, activebackground=BOX_COLOR, highlightbackground=BOX_COLOR)
    color_line_blue.place(x=CV_WIDE + 25, y=285)

    color_line_red = Radiobutton(text="Красный", font="-family {Consolas} -size 14", variable=option_color, value=2,
                                 bg=BOX_COLOR, activebackground=BOX_COLOR, highlightbackground=BOX_COLOR)
    color_line_red.place(x=CV_WIDE + 280, y=250)

    color_line_background = Radiobutton(text="Зелёный", font="-family {Consolas} -size 14", variable=option_color,
                                        value=4, bg=BOX_COLOR, activebackground=BOX_COLOR,
                                        highlightbackground=BOX_COLOR)
    color_line_background.place(x=CV_WIDE + 280, y=285)

    # Line
    back_box_line = Label(text="", font="-family {Consolas} -size 16", width=35, height=7, bg=BOX_COLOR)
    back_box_line.place(x=CV_WIDE + 20, y=370)

    line_text = Label(win, text="Нарисовать отрезок", width=35, font="-family {Consolas} -size 16", bg=MAIN_TEXT_COLOR)
    line_text.place(x=CV_WIDE + 20, y=370)

    # Explain box
    explain_box_line = Label(text="", font="-family {Consolas} -size 14", width=25, height=6, bg=BOX_COLOR)
    explain_box_line.place(x=CV_WIDE + 450, y=210)

    explain_text = Label(win, text="Информация", width=25, font="-family {Consolas} -size 14", bg=MAIN_TEXT_COLOR)
    explain_text.place(x=CV_WIDE + 450, y=210)

    # Explain text
    explain_text = Label(text="Система координат: декартовая.\n"
                              "Центр (0, 0) находится в середине.\n"
                              "Единицы (целочисленные), в которых\n"
                              "задаются координаты точек и длина\n"
                              "отрезка - пиксели.", font="-family {Consolas} -size 9", bg=BOX_COLOR, anchor='w')
    explain_text.place(x=CV_WIDE + 450, y=250)
    ...
    y1_line_text.place(x=CV_WIDE + 250, y=405)

    y1_line = Entry(font="-family {Consolas} -size 14", width=9)
    y1_line.place(x=CV_WIDE + 310, y=405)
    ...
    y2_line_text.place(x=CV_WIDE + 250, y=455)

    y2_line = Entry(font="-family {Consolas} -size 14", width=9)
    y2_line.place(x=CV_WIDE + 310, y=455)

    draw_line_btn = Button(win, text="Нарисовать", font="-family {Consolas} -size 14",
                           command=lambda: parse_line(option.get(), option_color.get()), width=15, bg=TEXT_COLOR)
    draw_line_btn.place(x=CV_WIDE + 170, y=490)

    # Spektr
    back_box_spektr = Label(text="", font="-family {Consolas} -size 16", width=40, height=6, bg=BOX_COLOR)
    back_box_spektr.place(x=CV_WIDE + 20, y=550)

    line_text = Label(win, text="Нарисовать спектр", width=40, font="-family {Consolas} -size 16", bg=MAIN_TEXT_COLOR)
    ...
    angle_text.place(x=CV_WIDE + 320, y=590)

    angle = Entry(font="-family {Consolas} -size 14", width=9)
    angle.place(x=CV_WIDE + 400, y=590)

    draw_spektr_btn = Button(win, text="Нарисовать", font="-family {Consolas} -size 14",
                             command=lambda: parse_spektr(option.get(), option_color.get()), width=15, bg=TEXT_COLOR)
    draw_spektr_btn.place(x=CV_WIDE + 180, y=640)

    # Control buttons

    compare_time_btn = Button(win, text="Сравнить\nвремя", font="-family {Consolas} -size 14",
                              command=lambda: time_measure(), width=15, height=2, bg=TEXT_COLOR)
    compare_time_btn.place(x=CV_WIDE + 520, y=400)

    compare_steps_btn = Button(win, text="Сравнить\nступенчатость", font="-family {Consolas} -size 14",
                               command=lambda: steps_measure(), width=15, height=2, bg=TEXT_COLOR)
    compare_steps_btn.place(x=CV_WIDE + 520, y=470)

    clear_win_btn = Button(win, text="Очистить экран", font="-family {Consolas} -size 14",
                           command=lambda: clear_canvas(), width=15, height=2, bg=TEXT_COLOR)
    clear_win_btn.place(x=CV_WIDE + 520, y=540)
    ...
    x1_line.insert(END, "0")
    y1_line.insert(END, "0")

    x2_line.insert(END, "100")
    y2_line.insert(END, "-200")
