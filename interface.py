...
def set_figure_center(x_c, y_c):
    ...
    figure_c = Label(win, text="Центр фигуры: (%3.2f;%3.2f)" % (x_c, y_c), width=32, font=("Arial", 14),
                     bg=TEXT_COLOR)
...
# Figure center
figure_c = Label(win, text="Центр фигуры: (%3.2f;%3.2f)" % (x_all[0][0], y_all[0][0]), font=("Arial", 14), width=36,
                 bg=TEXT_COLOR)
figure_c.place(x=CV_WIDE + 1, y=850)

# Center
center_label = Label(win, text="Центр (для масштабирования и поворота)", font=("Arial", 14), bg=WIN_COLOR)
center_label.place(x=CV_WIDE + 15, y=20)

center_x_label = Label(win, text="X:", font=("Arial", 14), bg=WIN_COLOR)
center_x_label.place(x=CV_WIDE + 70, y=50)
center_x = Entry(win, font=("Arial", 14), width=9)
center_x.insert(END, "0")
center_x.place(x=CV_WIDE + 100, y=50)

center_y_label = Label(win, text="Y:", font=("Arial", 14), bg=WIN_COLOR)
center_y_label.place(x=CV_WIDE + 270, y=50)
center_y = Entry(win, font=("Arial", 14), width=9)
center_y.insert(END, "0")
center_y.place(x=CV_WIDE + 300, y=50)

# Spin
spin_label = Label(win, text="Поворот", width=36, font=("Arial", 18), bg=TEXT_COLOR)
spin_label.place(x=CV_WIDE + 1, y=110)

spin_angle_label = Label(win, text="Угол°: ", font=("Arial", 14), bg=WIN_COLOR)
spin_angle_label.place(x=CV_WIDE + 160, y=155)
spin_angle = Entry(win, font=("Arial", 12), width=9)
spin_angle.insert(END, "0")
spin_angle.place(x=CV_WIDE + 240, y=155)

spin_btn = Button(win, text="Повернуть", font=("Arial", 14), command=parse_spin, width=15,
                  height=2, bg=TEXT_COLOR)
spin_btn.place(x=CV_WIDE + 160, y=200)

# Scale
scale_label = Label(win, text="Масштабирование", width=36, font=("Arial", 18), bg=TEXT_COLOR)
scale_label.place(x=CV_WIDE + 1, y=300)

scale_x_label = Label(win, text="kx: ", font=("Arial", 14), bg=WIN_COLOR)
scale_x_label.place(x=CV_WIDE + 100, y=360)
scale_x = Entry(win, font=("Arial", 14), width=9)
scale_x.insert(END, "1")
scale_x.place(x=CV_WIDE + 140, y=360)

scale_y_label = Label(win, text="ky: ", font=("Arial", 14), bg=WIN_COLOR)
scale_y_label.place(x=CV_WIDE + 270, y=360)
scale_y = Entry(win, font=("Arial", 14), width=9)
scale_y.insert(END, "1")
scale_y.place(x=CV_WIDE + 310, y=360)

scale_btn = Button(win, text="Масштабировать", font=("Arial", 14), command=parse_scale,
                   width=15, height=2, bg=TEXT_COLOR)
scale_btn.place(x=CV_WIDE + 160, y=420)

# Move
move_label = Label(win, text="Перемещение", width=36, font=("Arial", 18), bg=TEXT_COLOR)
move_label.place(x=CV_WIDE + 1, y=520)

move_x_label = Label(win, text="dx: ", font=("Arial", 14), bg=WIN_COLOR)
move_x_label.place(x=CV_WIDE + 100, y=580)
move_x = Entry(win, font=("Arial", 14), width=9)
move_x.insert(END, "0")
move_x.place(x=CV_WIDE + 140, y=580)

move_y_label = Label(win, text="dy: ", font=("Arial", 14), bg=WIN_COLOR)
move_y_label.place(x=CV_WIDE + 270, y=580)
move_y = Entry(win, font=("Arial", 14), width=9)
move_y.insert(END, "0")
move_y.place(x=CV_WIDE + 310, y=580)

move_btn = Button(win, text="Передвинуть", font=("Arial", 14), command=parse_move,
                  width=15, height=2, bg=TEXT_COLOR)
move_btn.place(x=CV_WIDE + 160, y=640)

line = Label(win, text="", width=36, font=("Arial", 18), bg=TEXT_COLOR)
line.place(x=CV_WIDE + 1, y=710)

stab_back = Button(win, text="Шаг назад", font=("Arial", 14), command=step_backing,
                   width=15, height=2, bg=TEXT_COLOR)
stab_back.place(x=CV_WIDE + 25, y=760)

clear = Button(win, text="Сбросить", font=("Arial", 14), command=reset, width=15,
               height=2, bg=TEXT_COLOR)
