...
def get_vector(first_dot, second_dot):
    return [second_dot[X_DOT] - first_dot[X_DOT], second_dot[Y_DOT] - first_dot[Y_DOT]]
...
def get_normal(first_dot, second_dot, pos):
    f_vect = get_vector(first_dot, second_dot)
    pos_vect = get_vector(second_dot, pos)
...
def cyrus_beck_algorithm(line, count):
    ...
    d = [second_dot[X_DOT] - first_dot[X_DOT], second_dot[Y_DOT] - first_dot[Y_DOT]]
    ...
    for i in range(-2, count - 2):
        ...
        w = [first_dot[X_DOT] - cutter[i][X_DOT], first_dot[Y_DOT] - cutter[i][Y_DOT]]
    ...
    first_dot_res = [round(first_dot[X_DOT] + d[X_DOT] * t_bottom), round(first_dot[Y_DOT] + d[Y_DOT] * t_bottom)]
    second_dot_res = [round(first_dot[X_DOT] + d[X_DOT] * t_top), round(first_dot[Y_DOT] + d[Y_DOT] * t_top)]

    res_color = parse_color(option_color_cut_line.get())

    if t_bottom <= t_top:
        canvas_win.create_line(first_dot_res, second_dot_res, fill=res_color)
...
if __name__ == "__main__":
    ...
    make_figure_btn = Button(win, text="Замкнуть отсекатель", font="-family {Consolas} -size 14",
